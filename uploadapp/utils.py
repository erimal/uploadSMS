...
from .dbSettings import LOG_FILENAME
#from .dbConfig import add_sms, deliver_message
...
class Orange:
    ...
    def send_SMS(phone, user_id):
        ...
        file_log = LOG_FILENAME + "_" + (datetime.now().strftime("%Y-%m-%d"))
        ...
        try:
            ...
            token = "Bearer " + Orange.get_orange_token('URL')
            ...
            if response.status_code == 201:
                ...
                row['message'] = msg
                row['id'] = Orange.random_with_N_digits(5)
                print("utils:send_orange_SMS: other ID ", row['id'])
                db.add_sms(row, user_id)
                db.deliver_message(row, resource_id, user_id)
            else:
